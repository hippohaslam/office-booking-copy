...
try {
    ...
    [void]$PromptBuilder.AppendLine("    * **Artifact Management**: Utilize `actions/upload-artifact@v4` to store build outputs. When creating a .zip file from a directory's contents, the zip archive **must** contain the files at its root, not nested inside a parent folder. The correct command pattern is `cd ./publish-directory && zip -r ../archive.name.zip .`. When creating deployment versions (e.g., for Elastic Beanstalk), the version label **must** be unique across all runs, including re-runs. A robust pattern is `${{ github.sha }}-`${{ github.run_number }}-`${{ github.run_attempt }}`.")
    [void]$PromptBuilder.AppendLine("")
    ...
    [void]$PromptBuilder.AppendLine("    * Always utilize the latest stable and officially supported versions of GitHub Actions (e.g., `actions/checkout@v4`, `actions/setup-dotnet@v4`, `actions/setup-node@v4`) and integrate with cloud providers using their recommended, up-to-date methods. When using a CLI command, ensure that all parameters, such as `--job-type` for AWS Amplify, use values appropriate for the intended action (e.g., use `RELEASE` for deploying a new version, not `MANUAL`).")
